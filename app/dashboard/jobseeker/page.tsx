...
export default function JobseekerDashboardPage() {
  ...
  const [savedJobs, setSavedJobs] = useState<JobItem[]>([]); // placeholder (no backend route provided)
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const fields = ["basic", "details"]; // heuristic for now
      ...
      // Saved jobs placeholder: local cache fallback
      const saved = JSON.parse(localStorage.getItem("saved_jobs") || "[]");
      setSavedJobs(saved);
    } catch (e) {
      // Fail silently per widget to avoid blocking page
